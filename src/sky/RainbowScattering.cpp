...
{
    if (params.angleBins < 16 || params.effectiveRadiusMicrometers <= 0.0 || params.effectiveVariance < 0.0 || params.solarAngularRadiusRadians < 0.0) {
    ...
    for (int band = 0; band < kSpectralBandCount; ++band) {
        ...
        const double geometricRadiusUm = params.effectiveRadiusMicrometers / std::exp(2.5 * sigmaLn * sigmaLn);
        ...
        for (int i = 0; i < bins; ++i) {
            ...
            if (!std::isfinite(normalizedF11) || normalizedF11 < 0.0 || std::abs(normalizedF12) > normalizedF11 * 1.000001 || std::abs(normalizedF33) > normalizedF11 * 1.000001)
    ...
    const auto primaryPeak = [&](int band) {
        ...
        for (int i = primaryFirst + 1; i <= primaryLast; ++i)
            if (table[static_cast<size_t>(band * bins + i)].f11 > table[static_cast<size_t>(band * bins + peak)].f11)
                peak = i;
        return peak;
    };
